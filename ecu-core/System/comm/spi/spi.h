 ...
 *
 * @param handle SPI Bus device handle
 * @param csPinBank GPIO Bank for chip select pin
 * @param csPin Pin number for chip select pin
 * @return Return status. SPI_STATUS_OK for success. See SPI_Status_T for more.
 */
SPI_Status_T SPI_RegisterCallback(
    const SPI_HandleTypeDef* handle,
    const GPIO_TypeDef* csPinBank,
